...
#"LSL: Lesion size-based loss"
    ...
    def forward(self, x, y, loss_mask=None):
        ...
        gt = y.long()
        ones = (gt == 1.).sum(dim=[1,2,3,4])
        
        
#        "size y output: 2,3,img_size--> 2 number of batch(slice), 3 number of classes
#        "we have 2 images as batch"
       
#        "first_pic = gt[0, 0, :]"
#        "second_pic = gt[1, 0, :]"
#        "determine the number of pixels equal to 1"
#        "f_ones = (first_pic == 1.).sum(dim=[0,1,2])"
#        "s_ones = (second_pic == 1.).sum(dim=[0,1,2])"
        ...
        ones_vol = torch.full_like(volumes,sum_ones).to("cuda")
        ...
        #fn = fn / volumes
        first_pic0 = gt[0, 0, :]
        f_ones0 = (first_pic0 == 1.).sum(dim=[0,1,2])
        print("f_ones0")
        print(f_ones0)
        
        first_pic1 = gt[0, 1, :]
        f_ones1 = (first_pic1 == 1.).sum(dim=[0,1,2])
        print("f_ones1")
        print(f_ones1)
        
        first_pic2 = gt[0, 2, :]
        f_ones2 = (first_pic2 == 1.).sum(dim=[0,1,2])
        print("f_ones2")
        print(f_ones2)
